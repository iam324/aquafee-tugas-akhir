// app_httpd.cpp
// File ini menyediakan HTTP server untuk streaming kamera ESP32-CAM
// Fungsi startCameraServer() dipanggil dari feeder_esp32.ino
...
// Handler untuk snapshot (1 frame JPEG)
...
void startCameraServer() {
  ...
  if (httpd_start(&camera_httpd, &config) == ESP_OK) {
    ...
    Serial.println("Camera HTTP server dimulai di port 80 -> /capture");
  }
  ...
  if (httpd_start(&stream_httpd, &config) == ESP_OK) {
    ...
    Serial.println("Stream HTTP server dimulai di port 81 -> /stream");
  }
}
